...
def _mac_to_int(address: str) -> int:
    cdef bytes encoded = address.encode("ascii")
    cdef const char *buf = encoded
    cdef size_t length = len(encoded)
    cdef uint64_t result = _bdaddr_to_uint64(buf, length)
    if result == UINT64_MAX:
        # Sentinel: fall back to Python so callers get a ValueError that
        # describes the actual offending input.
